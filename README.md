...
系统在真实书写场景下的运行界面如下图所示：

<p align="center">
  <img src="docs/images/demo_digit_recognition.png" width="48%" alt="手写数字序列识别与定格界面">
  <img src="docs/images/demo_multiline_letters.png" width="48%" alt="手写多行字母识别与大小写智能纠错">
  <br>
  <em>图 1：手写字符识别桌面工作台实测运行（左：单行手写数字实时分割与识别；右：多行手写字母序列分割与纠错后处理）</em>
</p>
...
### 3.3 系统实时性能与各阶段延迟基准 (Latency Breakdown)

为确保桌面交互工作台保持流畅响应，系统对全处理流水线进行了毫秒级基准测试（测试环境：Intel/AMD 标压 CPU + 常规 USB 摄像头，输入图像分辨率 $1280 \times 720$）：

| 处理阶段 | 核心操作与算法 | 平均耗时 (ms) | 占比 | 优化机制 |
| :--- | :--- | :---: | :---: | :--- |
| **阶段一：图像预处理** | 背景光高斯差分除法、极性自适应检测、全局 Otsu 二值化 | $8.2 \text{ ms}$ | $28.3\%$ | OpenCV 矩阵并行除法与边缘采样 |
| **阶段二：字符空间分割** | 形态学闭运算、连通域提取、迭代边界框合并与重心对齐 | $6.5 \text{ ms}$ | $22.4\%$ | 启发式空间几何约束与零阶/一阶矩平移 |
| **阶段三：CNN 神经网络推理** | 10~15 个单字切片并行 Batch 推理（含 TTA 变体多采样） | $14.1 \text{ ms}$ | $48.6\%$ | SiLU 融合算子加速、模型预热与 Batch 吞吐 |
| **阶段四：后处理语义纠错** | MAP 联合概率对数打分、长宽比/高度比校验与正则掩码 | $0.2 \text{ ms}$ | $0.7\%$ | $O(1)$ 长度过滤与混淆编辑距离剪枝 |
| **系统总端到端耗时** | **从按下定格识别到界面卡片高保真文本输出** | **$\approx 29.0 \text{ ms}$** | **$100\%$** | **低于单帧时长，前端稳定保持 $30\text{ FPS}$** |
