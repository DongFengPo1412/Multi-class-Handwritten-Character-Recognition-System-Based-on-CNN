...
其中 $G_{\sigma}$ 为标准差 $\sigma = 51$ 的高斯平滑核，整个除法操作利用 OpenCV 的 `cv2.divide` 矩阵并行执行，可还原出不受光照影响的白底黑字笔画。

#### 2.1.2 自适应对比度极性检测
为使系统自动适应“纸张书写（亮底暗字）”与“黑板板书（暗底亮字）”等不同媒介，系统在二值化后自动提取图像最外层边缘的特征像素集合：
$$\Gamma = \text{Border}(\text{Thresh})$$
统计边缘像素强度的期望值 $E[\Gamma]$。当 $E[\Gamma] > 127$ 时，判定背景为亮色，此时为使字符特征契合神经网络的输入分布，将图像进行取反操作：
$$\text{Thresh}_{\text{input}}(x, y) = 255 - \text{Thresh}(x, y)$$
否则，直接保持原样输入（即保持字迹为白色，背景为黑色）。该算法实现了全自动的环境极性转换。
...
#### 2.2.1 形态学闭运算笔画桥接
手写用力不均或二值化阈值偏高会导致笔画断裂，若直接寻找轮廓会导致单个字符被分割。系统在提取轮廓前，对二值化图先应用一个 $2 \times 2$ 矩形结构元 $S$ 执行闭运算（Closing Operation）：
$$\text{Closed} = (\text{Thresh} \oplus S) \ominus S$$
该操作能够填补笔画内部的细小空洞，并桥接小于 2 像素的笔画断裂，提高后续轮廓提取的连贯性。

#### 2.2.2 迭代式边界框合并（Box Merging）
常规字符分割只进行单次顺序扫描合并，极易遗漏非相邻的笔画碎片。系统设计了基于多轮迭代的边界框合并算法，通过自适应启发式准则 `should_merge(box1, box2)` 重复迭代，直至框数量收敛。
...
将缩放至 $20 \times 20$ 像素的字符图像放置在 $28 \times 28$ 像素的标准画布上，计算其重心与画布几何中心 $(14, 14)$ 的位移向量：
$$[\Delta x_s, \Delta y_s] = [14.0 - x_c, 14.0 - y_c]$$
应用仿射矩阵将图像平移对齐，消除了手写体带来的平移偏置。
...
手写形近字符（如 `hello` 易被 CNN 错识别为 `he11O`）是纯视觉分类的瓶颈。当系统检测到序列属于英文单词语境时，在常用英文单词集合 $D_L$（包含 10,000 个常用单词）中进行概率对数似然打分，寻找最大后验概率单词 $W^*$：
$$W^* = \arg\max_{W \in D_L} \sum_{i=1}^{N} \ln \left( P(c_i^{\text{lower}} \mid x_i) + P(c_i^{\text{upper}} \mid x_i) \right)$$
其中 $P(c_i \mid x_i)$ 为第 $i$ 个字符位置对应的 TTA 预测 Softmax 概率。通过对数相加代替乘法，避免了概率数值下溢，提高了纠错算法的稳定性。

#### 2.4.2 长宽比几何校验区分 `0` 与 `O/o`
对于形状高度近似的数字 `0` 和字母 `O/o`，系统在特征维度上引入了长宽比校验：
$$\text{AR}_i = \frac{w_i}{h_i}$$
统计表明，手写数字 `0` 笔画通常偏窄，而字母 `O` 偏圆。当识别结果产生 `0` 与 `O` 的歧义时：
* 若 $\text{AR}_i < 0.52$，对数字 `0` 的输出概率乘以 $1.5$ 予以奖励，对字母 `O` 与 `o` 的概率乘以 $0.05$ 予以惩罚。
* 若 $\text{AR}_i \ge 0.52$，则对字母 `O` 和 `o` 赋予加权，倾向于判定为字母。

#### 2.4.3 行内相对高度比大小写纠正
EMNIST 数据集内大小写字形对称的字母（如 `C/c`、`O/o`、`S/s`、`Z/z` 等）极易混淆。系统通过统计多字识别序列，构建了行内空间分布相对高度比纠错：
$$r_i = \frac{h_i}{\max_{j=1}^N (h_j)}$$
对于对称字符，若其空间相对高度比 $r_i < 0.78$，则将其纠正映射为其对应的小写字母，反之判定为大写字母。
...
#### 2.5.1 引入原因（对比实验对照组）
本地构建的字符识别系统在离线单字分类上具有高精度，但为了评估其实时手写文本行分割及单词拼写校正机制在实际表现中的准确率，系统引入了百度 AI 开放平台的手写体 OCR API 作为**外部基准对照组**。
* **评估基准**：在用户触发识别时，系统同时输出本地 CNN 结果、拼写纠错后结果以及百度 OCR 的识别结果，以便开发者直观对比本地自定义网络与云端成熟商业服务的差异，明确本地算法的优化空间。
...
1. **凭证获取与缓存**：在系统启动时，`BaiduOCRClient` 读取环境变量或本地 `baiduocr/key.txt` 配置文件中的 `API Key` 和 `Secret Key`，向百度鉴权服务器发起 OAuth 2.0 请求，获取并缓存为期 30 天的 `access_token`。
2. **图像编码与发送**：当触发识别任务时，系统从当前相机帧中截取红框选定的 ROI 图像区域，利用 OpenCV 进行 JPEG 格式内存编码：
   $$\text{Img}_{\text{JPEG}} = \text{imencode}(\text{ROI})$$
   随后转为 Base64 字符串，构造表单通过 HTTPS POST 请求发送至接口地址 `https://aip.baidubce.com/rest/2.0/ocr/v1/handwriting`。
3. **异步并发请求**：为了避免云端网络请求带来阻塞，系统在 Tkinter GUI 中采用后台工作线程发送请求，接收返回的 JSON 数据，解析出整行文本与平均置信度后，更新界面对照卡片。
...
* **带标签平滑的交叉熵损失**：
  ...
  设置平滑因子 $\alpha = 0.1$，类别数 $K = 62$。该设计降低了网络对样本硬分类的过拟合倾向，提升了模型对潦草边缘字迹的鲁棒性。
* **参数配置**：使用 Adam 优化器，学习率初始值 $\eta = 10^{-3}$，权重衰减参数（L2 正则化）设置为 $10^{-4}$。
* **负反馈调度与早停**：引入 `ReduceLROnPlateau` 调度机制，若验证集 Loss 连续 3 轮未降低则学习率减半；连续 7 轮不提升则触发早停以结束训练。
...
* **学术数据增强手段**：针对摄像头捕捉和签字笔笔画特征，在加载器中集成了全维度数据增强：
  1. **视角矫正**：因原始数据矩阵被转置，需将图像进行 $-90^{\circ}$ 旋转并水平翻转。
  2. **随机仿射变换**： degrees=$\pm 15^{\circ}$ 旋转、平移比率为 $12\%$、缩放范围 $0.8 \sim 1.2$、剪切角为 $12^{\circ}$。
  3. **透视变换**：RandomPerspective（畸变系数为 $0.2$，概率为 $0.4$），模拟相机偏角。
  4. **弹性形变**：RandomApply + ElasticTransform（系数 $\alpha = 50.0$），模拟纸张起伏。
  5. **模糊与噪声**：添加高斯模糊（核大小为 $3$）与随机擦除（RandomErasing，遮蔽面积 $2\% \sim 10\%$）。

---

## ⚡ 4. UI 交互设计与并发工程学

系统界面使用 Tkinter 构建，提供了一套具有动态反馈的一体化科学仪器交互面板。

### 4.1 异步多线程执行器（ThreadPoolExecutor）
* **痛点**：若识别推理与百度网络 API 请求都在 UI 主线程执行，当用户触发识别时，界面画面会瞬间停滞卡死（卡顿长达数秒）。
* **架构设计**：界面基于 `ThreadPoolExecutor` 线程池将计算任务与相机流渲染完全剥离。
  * **主线程**：维持 $30\text{ms}$ 的定时回调，驱动 OpenCV 读取摄像头视频，实时完成阴影补偿与二值化处理，在前端保持 $30\text{ FPS}$ 画面刷新。
  * **工作线程**：当用户按下空格键时，从主线程拷贝一份当前帧 ROI 区域图像，交由后台工作线程异步执行 CNN + TTA 推理与网络 OCR API 访问，计算完成后将结果回调刷新回 UI 组件，界面全程无停顿。
...
* **FREEZE 状态**：当按下空格键（或点击 Recognize）时，状态机跃迁为 `FREEZE`（目标徽章呈现橙色）。此时画面锁存，系统将在定格的图像红框区域上绘制分割出来的字符青色（Cyan）边界框，并在角标处绘制序号。
