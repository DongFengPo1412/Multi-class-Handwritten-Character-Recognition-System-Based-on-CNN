...
実環境での手書き文字認識実行画面は以下の通りです：

<p align="center">
  <img src="docs/images/demo_digit_recognition.png" width="48%" alt="手書き数字列の認識と画面静止表示">
  <img src="docs/images/demo_multiline_letters.png" width="48%" alt="手書き複数行英字の認識と大小文字補正">
  <br>
  <em>図 1：手書き文字認識デスクトップダッシュボードの実機実行画面（左：単一行手書き数字の空間分割と推論；右：複数行手書きアルファベットの分割とスペル補正結果）</em>
</p>
...
実環境下でカメラを使用する場合、手やスマートフォンの影が写り込み、単純な二値化では広範囲の黒ずみが発生します。本システムは背景照明差分除去アルゴリズムによりこの課題を解決します。まず大核ガウシアンフィルタを用いて背景の局所的な照度マップ（Illumination Map）を推定し、除算処理を行います。
...
### 3.3 システムのリアルタイム性能と各処理段階の遅延プロファイル (Latency Breakdown)

デスクトップダッシュボードのスムーズな対話応答を維持するため、全処理パイプラインに対してミリ秒単位のプロファイリングを実施しました（評価環境：Intel/AMD 標準電圧CPU ＋ 市販USBカメラ、入力解像度 $1280 \times 720$）：

| パイプライン処理段階 | 主要アルゴリズム・演算内容 | 平均所要時間 (ms) | 割合 | 最適化アプローチ |
| :--- | :--- | :---: | :---: | :--- |
| **第1段階：画像前処理** | 背景光ガウス差分除算、極性自動検出、大津の二値化 | $8.2 \text{ ms}$ | $28.3\%$ | OpenCV 行列並列除算と境界サンプリング |
| **第2段階：空間文字分割** | 形態学的閉演算、連結成分抽出、繰り返しボックスマージ・重心整列 | $6.5 \text{ ms}$ | $22.4\%$ | 空間幾何ヒューリスティクス制約と画像モーメント移動 |
| **第3段階：CNN推論** | 10〜15個の文字パッチ並列バッチ推論（TTA マルチサンプリング含む） | $14.1 \text{ ms}$ | $48.6\%$ | SiLU 融合演算子、モデルウォームアップとバッチ処理 |
| **第4段階：後処理言語補正** | MAP 対数尤度辞書スコア計算、縦横比/相対高さ比検証、正規表現マスク | $0.2 \text{ ms}$ | $0.7\%$ | $O(1)$ 長さフィルタリングと編集距離枝刈り |
| **システム全体エンドツーエンド遅延** | **認識キー押下から結果カードのテキスト描画完了まで** | **$\approx 29.0 \text{ ms}$** | **$100\%$** | **1フレーム描画時間を下回り、表示は $30\text{ FPS}$ を安定維持** |
