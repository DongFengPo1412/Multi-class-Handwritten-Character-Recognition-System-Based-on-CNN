...
実環境下でカメラを使用する場合、手やスマートフォンの影が写り込み、単純な二値化では広範囲の黒い汚れが発生します。本システムは背景照明差分除去アルゴリズムによりこの課題を解決します。まず大核ガウシアンフィルタを用いて背景の局所的な照度マップ（Illumination Map）を推定し、除算処理を行います。
...
#### 2.2.2 繰り返し境界ボックスマージ
従来の分割手法では単一方向の走査しか行わないため、隣接しないパーツの結合を見落としがちです。本システムは、適応型ルール `should_merge(box1, box2)` を用いた繰り返しマージアルゴリズムを採用し、ボックスの個数が収束するまで複数ラウンドにわたって処理を実行します。
...
手書き特有の筆跡のブレによる予測偏りを防ぐため、推論時に TTA マルチサンプリング融合機構を導入しています。
...
このテスト時アンサンブルによりノイズを効果的に平滑化し、識別器の抗噪性能を向上させます。
...
ここで $P(c_i \mid x_i)$ は、各位置 $i$ で予測された TTA Softmax 確率値を表します。確率の積ではなく対数の和を計算することで、アンダーフローを防ぎ、安定した辞書校正が可能となります。
...
開発したローカル認識システムは単一文字の分類に優れた精度を持ちますが、実環境での連続テキストのセグメンテーションや、後処理によるスペル修正の性能を客観的に評価するために、百度 AI の手書き文字 OCR 接口を**外部対照グループ**として導入しました。
* **精度比較**：ユーザーが認識をトリガーした際、ローカルの生のCNN出力、スペル修正後の出力、および百度OCRの出力を同時に表示します。これにより、ローカルのセグメンテーション・言語校正モデルと成熟したクラウドサービスとの差異を定量的に比較評価できます。
...
1. **トークンの取得とキャッシュ**：アプリ起動時、`BaiduOCRClient` が環境変数または `baiduocr/key.txt` から API キーとシークレットを取得し、OAuth 2.0 経由で Access Token を取得・キャッシュします。
2. **画像エンコードと送信**：認識開始時、切り出された ROI 領域の画像を JPEG 形式に圧縮エンコードし、Base64 文字列に変換した上で、`https://aip.baidubce.com/rest/2.0/ocr/v1/handwriting` に対して HTTP POST リクエストを送信します。
3. **非同期リクエスト処理**：通信によるGUIの遅延を防ぐため、リクエスト処理はバックグラウンドスレッドプールで実行され、返されたJSONからテキスト情報と信頼度をパースし、非同期でUIに反映します。
...
* **学習率スケジューラと早期終了**：`ReduceLROnPlateau` を採用し、検証損失が3エポック連続で停滞した場合に学習率を半分にします。また、検証精度が7エポック連続で停滞した場合は自動的に学習を終了します。
...
* **データ拡張手法**：
  ...
  4. **ノイズとブラー**：ガウシアンブラー（核サイズ $3$）とランダム消去（RandomErasing、比率 $2\% \sim 10\%$）。
...
* **解決策**：`ThreadPoolExecutor` を用いて計算処理をGUIスレッドからデカップリングしました。
  ...
  * **ワーカースレッド**：ユーザーがスペースキーを押すと、現在の関心領域（ROI）をコピーしてバックグラウンドで CNN + TTA 推論と API リクエストを処理。結果が得られ次第、コールバックを介してUIに反映するため、カクつきは一切発生しません。
